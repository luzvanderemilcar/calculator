...
function signCleanUp(str) {
  ...
  let strChange = str.replace(doubleSignRegex, "+");

  // handle multiple signs to the last one
  const signCleanUpRegex = /([+/*-]*)((?=([+/*][-]{0,1}[0-9.]+)))/g;

  return strChange.replace(signCleanUpRegex, "");
}

function calculateString(string) {
  let sanitizedString = string.replace(/[^-()\d\/+*.]/g, '');
  return eval(signCleanUp(sanitizedString));
}

function testDecimalPoint(str) {
  let arrNumbers = str.split(/[+*/-]/g);
...
class Calculator extends React.Component {
  ...
  inputChange(e) {
    ...
    this.setState(state => {
      ...
      if (state.input != "0") {
        ...
        if (data == "." && !testDecimalPoint(state.input)) {
          return {
            input: state.input + data,
            resultNum: '' };
        ...
        if (numRegex.test(data) || opeRegex.test(data)) {
          return {
            input: state.input + data,
            resultNum: '' };
  ...
  calculate() {
    this.setState(state => {
      if (state.input) {
        let result = calculateString(state.input);
        return {
